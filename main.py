...
from apartment_post_filter import ApartmentPostFilter, ApartmentFilter
from apartment_post_parser import ApartmentPostParser
...
facebook_groups_scraper = FacebookGroupsScraper(config.FACEBOOK_USERNAME, config.FACEBOOK_PASSWORD)
...
async def main():
    ...
    with open('app.log', 'r') as f:
        logs = '\n'.join(f.readlines())
        await bot.send_message(text=logs, chat_id=config.TELEGRAM_BOT_APARTMENTS_LOGS_GROUP_CHAT_ID)
